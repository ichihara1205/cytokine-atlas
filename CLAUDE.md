...
| status | 意味 | サイト上の表示 |
|---|---|---|
| `verified` | 一次文献の**本文**または本人の実測で確認済み。出典あり | 通常表示 |
| `figure_read` | 文献の**図**から読み取った。出典と「どの図か」が必須 | 青枠＋「図から読取」バッジ |
| `inferred` | Claude が一般知識から書いた。未検証 | グレー＋点線枠＋「未検証」バッジ |
| `todo` | 未着手・要調査 | 空欄＋TODOリストに集計 |

`figure_read` は `verified` と分けること。本文なら「この文が根拠」と引用できるが、
図は目視解釈であり、多対多のネットワーク図で線を1本読み違えれば誤りが混入する。
同じバッジにすると精度の違いが消える。
`figure` フィールド（例: `figure: "Fig. 1"`）が空ならビルドを止める。
検算できない読み取りは根拠を示さずに verified を名乗る抜け道になるため。
...
- `assays.qpcr.primers` と `assays.antibody` は **`verified` 以外はサイトに出力しない**（ビルドで落とす）。
  `figure_read` も出力しない。配列や品番を図から読み取るのは危険度が高すぎる。
